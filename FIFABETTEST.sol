...
contract FIFABETTEST{
    ...
    struct Game{
        ...
        uint32 startTime;
        uint32 endTime;
        uint status; //0: init ; 1: can do bet ; 2: do bet end; 3: game end and result handled
    }

    address payable cto;  //the sponsor of game
    Game[] public games;
    mapping(uint32 => mapping(uint => mapping(string => uint256))) public betDatas; 
    ...
    function addGame(string memory _teamA, string memory _teamB, uint16 _point) public onlyCto canReset{
        game.teamA = _teamA;
        game.teamB = _teamB;
        game.point = _point;
